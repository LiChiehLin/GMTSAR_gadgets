...
if ($#argv != 2 && $#argv != 4) then
 echo " "
 echo "Usage: BOI_processing.csh Reference_prefix Coherence [inc_x inc_y]"
 echo " "
 echo "Example: BOI_processing.csh S1_20190704_135158_F2 0.5 [0.001 0.001]"
 echo " "
 ...
 echo ""
 echo "Note: "
 echo "1. Process this code at the mother folder of raw/ (where you process align_tops_esd.csh)"
 echo "2. suffix with ll is taken with coherence higher than the given coherence"
 echo "3. make sure you have trans.dat in your intf/*"
 echo "4. ddphase_disp is measured in meter"
 echo "5. Run this after p2p_S1_TOPS.csh"
 echo "6. [inc_x inc_y] set blank will use original separation to output grds"
 echo " (Should use 0.001 will be fine, otherwise grds will be too big, several GBs)"
 echo ""
 exit 1
endif
...
set W = `gmt gmtinfo ddphase_ll.dat -C | awk '{print $1}'`
set E = `gmt gmtinfo ddphase_ll.dat -C | awk '{print $2}'`
set S = `gmt gmtinfo ddphase_ll.dat -C | awk '{print $3}'`
set N = `gmt gmtinfo ddphase_ll.dat -C | awk '{print $4}'`
...
if ($#argv == 4) then
  ...
  set inc_x = `echo $inc_x2 - $inc_x1 | bc -l | awk '{print sqrt($1*$1)}'`
  ...
  set inc_y = `echo $inc_y2 - $inc_y1 | bc -l | awk '{print sqrt($1*$1)}'`
...
gmt psbasemap -R$W/$E/$S/$N -JM10c -Ba1f1 -BWSen -K > ddphase_ll.ps
gmt grdimage ddphase_ll.grd -Cddphase.cpt -R -J -O -K -P -Q -V >> ddphase_ll.ps
...
gmt psbasemap -R$W/$E/$S/$N -JM10c -Ba1f1 -BWSen -K > ddphase_pix_ll.ps
gmt grdimage ddphase_pix_ll.grd -Cddphase_pix.cpt -R -J -O -K -P -Q -V >> ddphase_pix_ll.ps
...
gmt psbasemap -R$W/$E/$S/$N -JM10c -Ba1f1 -BWSen -K > ddphase_disp_ll.ps
gmt grdimage ddphase_disp_ll.grd -Cddphase_disp.cpt -R -J -O -K -P -Q -V >> ddphase_disp_ll.ps
